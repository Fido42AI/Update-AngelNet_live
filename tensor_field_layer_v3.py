# === tensor_field_layer_v3.py ===
# TensorFieldLayer v3 с расширенной памятью, кластеризацией и защитой от ошибок размерности
...
class TensorFieldLayer(nn.Module):
    ...
    def _rotate_input(self, x):
        if x.shape[1] != self.in_features:
            print(f"[!] Warning: rotating tensor with shape {x.shape}, expected (?, {self.in_features})")
            x = x.expand(-1, self.in_features)  # временная адаптация
    ...
    def forward(self, x, external_feedback=None):
        if x.shape[1] != self.in_features:
            print(f"[!] Mismatched input shape: {x.shape}, expected (?, {self.in_features}). Padding...")
            x = x.expand(-1, self.in_features)
